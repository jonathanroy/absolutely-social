		...
		function asocial_insert_icons()
		{
			...
			foreach ( $asocial_sites as $key => $val ) {
				if ( isset($asocial_options[$key]) && $asocial_options[$key] == 'on' ) {
					...
					$icons[] .= "<a id=\"" . $key . "-icon\" class=\"asocial-icon\" href=\"" . $url . "\" onclick=\"return false; window.open('" . $url . "', '" . $val['name'] . "', 'toolbar=no,width=500,height=500');\" title=\"Share on " . $val['name'] . "\" target=\"_blank\">";
